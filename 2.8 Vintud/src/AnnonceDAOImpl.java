   ...
   public void Filtrer_annonces_prix(double max_price) {
	   ...
	   ConnectionManager C= ConnectionManager.getInstance();
		...
		try {
           ...
           ResultSet res = stmt.executeQuery("SELECT * FROM announcement  WHERE  price < "+max_price+" ORDER BY price  " );
           while(res.next()) {
               System.out.println("Cette annonce est identifiée par le nom "+" "+res.getString("title") +" "+ "et par la description"+" "+res.getString("description")+" "+"et par le prix"+" "+res.getDouble("price"));
               }
